class RoutelistController < ApplicationController
  ...
  def index
	...
	@visiting_route = VisitingRoute.find(:all, :conditions=>["salesman_id=? AND DATE(created_at)=?", @paramsalesmanid, Time.now.strftime("%Y-%m-%d")])
  ...
  def search
	...
	@visiting_route = VisitingRoute.find(:all, :conditions=>["salesman_id=? AND DATE(created_at)=?", @paramsalesmanid, @paramscreated_at])  
    respond_to do |format|
      format.html { render :index }
      format.json { render :partial => "routelist/index.json" }
    end
